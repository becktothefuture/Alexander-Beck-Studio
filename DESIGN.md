...
- The Home identity, Work introduction, About opening/finale, and Contact title share one viewport-stable bookend motion: after the destination view settles, individual letters resolve left to right from a soft blur and a restrained `-0.12em` horizontal offset. Where a title uses the shared lockup, its short rule then scales horizontally from the centre before the description resolves; action follows the readable lockup. Bookend motion never moves vertically, clips, or crops glyphs. All four identities use the shared `--route-bookend-title-size` at a `1.32` optical scale (approximately 15% smaller than the previous route endpoints) and shared tighter headline leading. Reduced motion removes blur, travel, line scaling, and stagger while preserving the complete hierarchy.
...
- The Work, Contact, and both About Me bookend title/description lockups share one short title-colour rule and one ordered entrance: title, centre-out rule, then description. Its line-to-description gap is a single shell-authored value across every instance.
...
- The Button Bar is the only primary navigation. Its moving pill is one shared object, not four independent selected backgrounds. All primary tab cells use their rendered label width plus inline padding; the pill follows that padded cell with a controlled rounded-rectangle radius.
...
- The six disciplines are named once through a camera-linked reveal projected from exactly six emphasized grid points. Scroll authors their one-by-one growth and label sequence while a broad viewport corridor keeps each reveal attached to the travelling field. Their labels share one exact dot-centred baseline with no per-label compensation. Desktop and mobile each own normalized field anchors, while colours remain fixed and unique in category order: `--ball-1`, `--ball-4`, `--ball-3`, `--ball-7`, `--ball-8`, `--ball-6`.
