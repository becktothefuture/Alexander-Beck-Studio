...
> **19 September 2026 — cinematic publication.** About uses simple Blender surfaces with browser-generated circles. The [surface specification](docs/plans/about-surface-authoring-20260919/PLAN.md) defines this ownership change. Technical checks do not establish Alexander's visual acceptance.

- Production publishes the complete About journey. Copy, typography and visibility controls remain development-only; URL parameters do not enable them in production.
...
- Blender owns camera positions, tangent-facing sightlines, bank, physical beat boundaries, surface geometry, per-object colour roles and supported animation. Browser code generates circles at one code-owned spacing, uses Home's shared ball size, and applies one browser-owned visibility corridor, then plays the validated export. The browser uses an aspect-based portrait lens cap of at least 105 degrees; camera poses and the scene remain authored in Blender.
- Reading chambers use paired upright panels near the rail, with open floor sections between them. The saved Blender scene opens in camera view with timeline playback enabled and named chapter markers. One camera-depth visibility corridor fades circles close to the camera and in the distance. All surfaces, including the final wall, use the same four thresholds saved in design-system.json. Do not use scene-specific or progress-based visibility switches.
- Native scrolling owns camera progress. Measured copy can extend its reading budget without shortening either tunnel. Holding scroll holds the camera; reversing scroll retraces it. Independent, pause-aware ambient time drives the moving assemblies and final wall.
- Every title's visible glyph block remains centred at the inner viewport's 50%/50%, including the opening and ending. Titles do not bank or translate during their lifecycle. Support and contact actions sit in their own area below. Full prose uses Geist in the central reading column. A shared theme-aware material contrast range keeps moving circles from erasing the text.
...
- Circles always face the camera and use Home's actual gradient atlas and active simulation palette. Sampling density remains code-owned. `rollercoasterProjection.js` applies one viewport-calibrated diameter-to-pitch target of 0.55 to all surfaces, using resize buckets and hysteresis. The shared Home sizing helper sets the radius at a fixed 8-WU reference depth; normal perspective applies nearer and farther away. Resizing and Home's mobile size rule apply to every circle, including the final grid. Changing visibility does not change size. Each Blender object has one of six palette-role materials, or a seventh `All colours` marker. The browser replaces roles with the current Home palette; the marker mixes the existing six colours and adds no seventh browser colour. Blender previews solid surfaces and their supported motion.
...
- The docked development controls edit website-owned copy, typography and the visibility corridor. Copy and type save to contents-about.json; the four visibility distances save to the canonical design-system.json runtime keys. Geometry, timing, motion and source identity remain read-only in the browser; edit them in Blender, save and export. A verified manifest publication refreshes the development view. Hash mismatches show a readable fallback, never the rejected scene.
