...
The Home switcher is a compact, text-only next action, not a selector or dialog trigger. Its visible label remains `Change effect`, and its height is 90% of the shared labelled-action height. The current effect name stays in the accessible name and polite status announcement instead of changing the visible action label. The button is unavailable while the simulation transaction commits; the surrounding simulation transition provides the feedback, so the control adds no label handoff or icon animation.
