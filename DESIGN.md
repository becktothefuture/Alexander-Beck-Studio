...
- The Utility Rail is integrated into the studio window's right edge: one continuous contour bends inward above the controls, wraps around their left side, and returns below. The rail has no separate outer border, background, or floating shadow; a shell-owned curved join carries soft edge light and blends into the existing vertical edge. Desktop stays vertically centred at `50svh` with `32px` visible controls. Mobile deliberately becomes quieter: `25px` visible controls, an `-11px` outward offset, and a centre at `76svh` in the viewport's bottom half. Coarse-pointer hit areas expand invisibly to `44px` without allowing the two controls to overlap. The **Utility Rail** panel owns separate desktop and mobile padding, corner-radius, size, and horizontal-position controls plus the mobile vertical anchor; both horizontal ranges run from `-160px` to `160px`, the mobile size runs from `22px` to `44px`, and its vertical anchor runs from `55%` to `90%`. The rail keeps its authored anchor where space permits; short viewports constrain it within the window and horizontal offsets cannot push it outside the viewport. The rail uses stable outer-shell ink and material in every theme, persists through route transitions, and never becomes part of route content or the Button Bar.
...
- Production displays **Coming soon.** The narrative is excluded at build time, with no hidden link, URL or browser-storage bypass. Development keeps the canonical Blender-authored spatial narrative. `public/config/contents-about.json` remains the sole authored About copy and client source. The narrative contracts below apply to that development experience until publication is approved.
- Blender owns one stable semantic role per exported face, generated passage component, spatial region, or atmospheric instance. The six roles are atmosphere, stone, steel, glass, signal, and organic. Export objects default to deterministic `mixed` assignment; `single` is an explicit object override; `authored-faces` preserves deliberate solid-body face work.
- Website code resolves those role identifiers through the active Home palette. Palette changes update the six About shader uniforms only; they do not rebuild geometry, rewrite point attributes, or allocate per frame. Blender preview swatches must be synchronised from the same code source and never become a second production palette.
- Round tunnels and square gates vary by complete ring or gate. Continuous surfaces use broad regions or bands, flat grids use ordered cells, and atmospheric fields use stable weighted mixtures. Individual circles never change role during motion.
- The opening field supports the first title pair, background prose, and experience. The round tunnel carries the next title pair. The longer terrain clearing supports disciplines and clients. Growing square gates carry another title pair and the final prose. The last pair accompanies the approach from below to the bust and platform. The archived floating-body collection is excluded from the active export; all seven semantic camera stages remain stable.
- One native scroll state owns copy, camera, scene progress, and the progress indicator on the same frame. Camera motion follows the Blender rail through one monotone distance map: gentle continuous travel while scrolling through prose and faster travel during title passages, with continuous speed at their boundaries. There is no independent camera lag or pointer offset. Scroll reversal retraces the same path; only ambient surface motion advances with time.
...
- The opening, midpoint, travelling editorial fields, career rows, discipline fields, client proof, and finale retain their authored size and entrance behavior. The opening and final lockups use the shared route-bookend language; the final title, description, and actions occupy a lower copy region, separated from the sculpture in a measured upper image region.
...
- The finale contains only a closed, solid-looking point platform and the upright, grounded bust. Blender owns its vertical axis, base pivot, contact, and facing direction. Its default ambient turn is bounded to ±8° over 32 seconds. The camera holds at the terminal scroll position while the turn continues. Responsive projection fits the sculpture above the measured text and actions; neither may overlap or crop.
- One scene-control registry defines labels, defaults, ranges, and writes into `contents-about.json`. Typography retains five roles. Reading clearance, title travel, surface population and coverage, ambient motion, and atmosphere have live apply, save, reload, and reset behavior. Diagnostic quality overrides remain session-only.
- Rendering profiles separate atmospheric points, solid surfaces, and the bust. Density selects a fraction of the selected quality budget; coverage sets the footprint independently. Opaque depth cores and smooth circle edges preserve closed surfaces. Only atmospheric points use independent drift; terrain deforms coherently and rigid surfaces preserve their shape.
- Fog resolves once per frame. Blender owns the default distance range; a website override applies throughout the journey until reset. Development preview bundles must match the saved source and validate camera and point hashes. Stale cache falls back to the validated canonical bundle; immutable bundle URLs prevent mixed exports.
