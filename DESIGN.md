...
The Home switcher is a labelled next action, not a selector or dialog trigger. Its visible label remains `Change effect` and pairs with a shuffle glyph so the outcome is clear before activation. The current effect name stays in the accessible name and polite status announcement instead of changing the visible action label. The button is unavailable while the simulation transaction commits; the surrounding simulation transition provides the feedback, so the control adds no label handoff or icon rotation.
