...
The Home switcher is a compact, text-only next action, not a selector or dialog trigger. Its visible label remains `Change effect`, its height is 90% of the shared labelled-action height, and balanced 16px inline padding fits the surface to the label without a reserved icon gap. The current effect name stays in the accessible name and polite status announcement instead of changing the visible action label. The button is unavailable while the simulation transaction commits; the surrounding simulation transition provides the feedback, so the control adds no label handoff or icon animation. On mobile, equal footer side tracks place the edge caption on the page centreline between the social controls and London time.
