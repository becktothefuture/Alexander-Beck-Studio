...
The Home switcher is a compact, text-only next action, not a selector or dialog trigger. Its visible label is the quiet utility instruction `CHANGE EFFECT`: 10.5px medium-weight Geist with `0.09em` tracking inside a 34px capsule with balanced 14px inline padding. A 44px invisible hit area preserves touch access around the smaller surface. The switcher uses a quieter theme-aware translucent fill and one fine inset edge without a drop shadow. Hover, focus, and press change only the material and ink; the control never lifts, nudges, scales, bounces, or springs. The current effect name stays in the accessible name and polite status announcement instead of changing the visible action label. The button is unavailable while the simulation transaction commits; the surrounding simulation transition provides the feedback, so the control adds no label handoff or icon animation. On mobile, equal footer side tracks place the edge caption on the page centreline between the social controls and London time.
