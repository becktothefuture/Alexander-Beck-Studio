...
- The six disciplines use one grid reading line and one stable editorial text position. Six equally spaced cells in one grid column pass through that line. Only the active cell grows and receives its discipline colour; only its matching name and description are visible. Both text elements use `--about-editorial-type-size`, with hierarchy from weight only. The presentation has no card, connector, counter, glow, per-item position, or screen-space projection. Colours remain fixed in category order: `--ball-1`, `--ball-4`, `--ball-3`, `--ball-7`, `--ball-8`, `--ball-6`.
