// Fluid Portfolio Editor
// Core Application Logic

import { loadRuntimeConfig } from '../../modules/utils/runtime-config.js';
import { applyWallFrameFromConfig, applyWallFrameLayout } from '../../modules/visual/wall-frame.js';
...
import * as SoundEngine from '../../modules/audio/sound-engine.js';
import { initGateOverlay } from '../../modules/ui/gate-overlay.js';
import { initContactGate } from '../../modules/ui/contact-gate.js';

const BASE_PATH = window.PORTFOLIO_BASE || 'portfolio/page/';
const CONFIG = {
  basePath: BASE_PATH,
  assetBasePath: `${BASE_PATH}assets/portfolio-pages/pages/`,
};
...
class PortfolioApp {
  ...
  async loadData() {
    try {
      const response = await fetch(`${CONFIG.basePath}data/portfolio.json`);
  ...
  renderSlides() {
    ...
    for (let slot = 0; slot < slotCount; slot += 1) {
      ...
      const imgSrc = project?.image
        ...
        : `${CONFIG.basePath}assets/portfolio-pages/folio-cover/cover-default.webp`;
      img.src = imgSrc;
      img.loading = 'lazy';
      img.draggable = false;
