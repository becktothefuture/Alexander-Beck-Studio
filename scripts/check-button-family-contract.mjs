...
test('the production switcher presents one stable, explicit action', () => {
  ...
  assert.match(
    sources.main,
    /\.simulation-focus-pill\.simulation-focus-switcher \{[\s\S]*?--abs-labelled-action-height: 36px;[\s\S]*?--abs-labelled-action-font-size: 0\.65625rem;[\s\S]*?--abs-labelled-action-pad-x: 20px;[\s\S]*?--abs-labelled-action-gap: 0;[\s\S]*?--simulation-switcher-ink-pressed:[\s\S]*?92%[\s\S]*?box-shadow: var\(--abs-soft-control-shadow-rest\);[\s\S]*?backdrop-filter:[\s\S]*?var\(--abs-soft-control-blur\)[\s\S]*?font-weight: 500;[\s\S]*?letter-spacing: 0\.09em;[\s\S]*?outline: none;[\s\S]*?\.simulation-focus-pill::after \{[\s\S]*?44px[\s\S]*?\.simulation-focus-pill\.simulation-focus-switcher\[data-advancing='true'\] \{[\s\S]*?box-shadow: var\(--abs-soft-control-shadow-pressed\);[\s\S]*?translate: 0 1px;[\s\S]*?\.simulation-focus-pill\.simulation-focus-switcher:focus-visible \{[\s\S]*?outline: none;[\s\S]*?\.simulation-focus-pill\.simulation-focus-switcher:active \{[\s\S]*?box-shadow: var\(--abs-soft-control-shadow-pressed\);[\s\S]*?translate: 0 1px;[\s\S]*?\.simulation-focus-pill__label \{[\s\S]*?translate: 0 -0\.5px;[\s\S]*?text-transform: uppercase;[\s\S]*?white-space: nowrap;[\s\S]*?\.simulation-focus-pill\.simulation-focus-switcher:is\(:active, \[data-advancing='true'\]\)[\s\S]*?\.simulation-focus-pill__label \{[\s\S]*?translate: 0 0\.5px;/,
  );
  assert.doesNotMatch(
    sources.switcher,
