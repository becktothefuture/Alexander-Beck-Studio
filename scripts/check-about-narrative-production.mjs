...
for (const file of textAssets) {
  ...
  assert.doesNotMatch(source, /["']data-about-publication["']\s*:\s*["']released["']/, `Production asset ${file} can render the unpublished About journey.`);
  forbidden.forEach((needle) => {
    assert.equal(source.includes(needle), false, `Production asset ${file} contains development-only marker: ${needle}`);
  });
}
...
console.log('PASS: About publication is held; the production bundle contains no About journey, authoring controls or diagnostics');
