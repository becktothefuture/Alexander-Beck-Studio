...
| **Geometry** | Host uses the **same inner-window rectangle** as the visible wall interior: `position: fixed` with `--shell-inner-window-top/right/bottom/left`, **`border-radius: var(--frame-inner-radius)`**, **`overflow: hidden`**. Those semantic variables are derived from the legacy Safari tint insets plus `var(--frame-border-width)`; bottom intentionally includes `--shell-bottom-dock-band` so public route tabs sit outside the wall. Same **corner-shape** inheritance as canvas (e.g. squircle when `html.abs-corner-shape-squircle`). |
