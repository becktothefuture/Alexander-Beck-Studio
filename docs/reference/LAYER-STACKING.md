...
`StudioShell.jsx` owns the physical window, overlay hosts, Home-only footer surface, Button Bar, and Utility Rail. Route content stays inside the studio window. The Button Bar is a shell-owned sibling below and outside the window, with its complete touch-target height reserved. It is outside `#abs-scene` so scene impact cannot move its faces, icons or labels. The Utility Rail is a shell-owned sibling attached to the studio-window right edge. Neither fixture may be covered by route content, loaders, gates, or a project sheet.
...
The Utility Rail forms a bay in the right window edge. `ShellUtilityRail` owns its resize-driven clip and a pointer-transparent SVG join at `99`, above the original backing and below the clipped window. The join replaces only the short edge segment behind the controls and blends into the existing reflection above and below; it does not replace the full frame shadow. Window overlays and the Work sheet inherit the bay clip. The controls keep their existing shell layer and input behavior.

The exterior reflection uses the same contour behind `#simulations`, with a solid `--frame-inner-surface` backing beneath the antialiased join. The default contact blend is a 1px light overlap with 3px blur, using the current inner-rim light strength in both themes. Surface Finish exposes shared Scene depth, Scene softness, Light intensity, and Light direction controls with separate light/dark profiles and material responses. At 100% and Even, they retain the approved reflection. Softness coordinates contact, inner-rim, and near/far blur with a small lower limit to prevent a sharp stripe. Direction places both inset and exterior light on the selected side without moving the contour. The original per-layer calibration stays in Fine tuning and is authored in `shell.surface`. The window interior uses broad white reflected light, with no dark perimeter stroke or dark inset shadow.
...
`#portfolio-sheet-host` is a sibling overlay host within `#abs-scene`, after route content. The open Work sheet covers the Work field and reaches the same studio-window boundary as the route surface; the separate Button Bar remains visible and receives input below it. Its project scroll cue is drawer-owned and independent of the Home footer. Preserve the host radius and clipping contract.
...
`#shell-persistent-route-ui-host` is a shell-owned sibling outside `#abs-scene` and `#app-frame`. Route instruments that must not disappear during scene boot or route-content transitions mount here, remain clipped to the complete studio-window viewport, and stay pointer-transparent.
