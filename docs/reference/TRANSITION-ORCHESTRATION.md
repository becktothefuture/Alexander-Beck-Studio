...
- Every primary shared-shell route handoff (Home, Work, About, Contact, and Lab) uses the certified persistent-backplane policy: `data-abs-route-loader-backdrop="preserve"` makes the loader visually transparent while the stable grain and in-window backplane remain continuously rendered. The loader still blocks route input and carries status/spinner state; registered route surfaces remain independently hidden. Unknown or standalone routes retain the opaque fallback.
...
- Home prewarming imports its route and active simulation-mode modules plus cached copy, but never allocates a canvas, initializes balls, or starts the render loop. Contact caches the design-system configuration. About prewarming imports its code-split playback module without mounting the point world or starting its renderer. Playground prewarming validates its catalogue and configuration without mounting the camera, dot renderer, video, or code runtimes.
...
- [ ] Work, About Me, Contact, and Lab returns to Home keep the title plane covered after commit, begin route-in from a staged blank title revision, and settle with one visible source-connected title plane.
