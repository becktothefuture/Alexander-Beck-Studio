...
- Home and route-backed Daily Focus loads may release only after fonts, usable route geometry, the active simulation surface, two render frames, and the title geometry source are ready. `data-abs-home-simulation-ready` and `data-abs-home-canvas-title-prepared` expose that gate for diagnostics.
...
- Unresolved Home canvas/title readiness has a 45-second hard deadline so the 40-second reassurance can appear; known failures fail open immediately. Both paths settle any available material and record a failed readiness state instead of leaving an infinite loader.
