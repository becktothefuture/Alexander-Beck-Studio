...
- The first-paint loader is a compact 36px six-dot spinner derived from the six canonical `colorDistribution` ball positions, with boot-specific dot variables so low-contrast ball colours do not disappear on the boot chrome. The dots use the loader playground's Fluid Sweep cadence: a roughly 2.24s eased orbit and 4.8px dots on a 14.25px radius. Colour changes are intentionally less frequent: the dots hold a stable palette through most of a roughly 6.72s colour cycle, then run a short stepped burst around the ring using 160ms phase offsets. Saved light and dark themes use separate boot dot palettes; both palettes must stay at or above 3:1 contrast against their intended boot backgrounds.
