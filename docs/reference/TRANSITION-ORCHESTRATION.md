...
- `route-transition-loader-timing.js` owns the content-visibility barrier timestamp and destination paint barriers. Its input/status shield stays transparent; it adds no spinner delay or minimum hold and does not own global phase.
...
- Cold destination preparation retains the current scene in `idle`; the visible exchange then starts with `route-out`.
- Content visibility protection, the transparent input shield and committed-route readiness barrier use `route-loading`.
...
- The persistent Home title plane may retain its last valid bitmap for remount and resize continuity, but retained or readiness pixels are never presentation authority. Every route transaction that commits or restores Home covers the title plane with a generation-scoped presentation gate, stages the shared title entrance, waits for a newer source-connected blank Canvas revision, and releases the gate in the same task that publishes `route-in`.
...
- Normal route navigation is one shell-owned transaction: cold module and data preparation retains the visible outgoing scene, then `route-out` fades its owned surfaces. The shell protects destination visibility before provisional commit, waits for layout readiness and staged paint behind the transparent input/status shield, then starts `route-in` and commits durable history. No visible loader or loading plate enters this exchange. A superseded generation may never commit, reveal, focus, announce, or clean up a newer transaction.
...
- The Home pill advances exactly one entry through the catalog's circular `daily-rotation` order. It never mounts a selector, dialog, backdrop, or modal blur.
- The `out|commit|prime|in` handoff stays unobscured so the material scale-down/scale-up transition remains visible.
...
- Simulation handoff state must never paint a modal blur on Home or affect Work, About, Contact, or another route if state becomes stale.
...
- Development Work prepares its spatial world behind the overlay and releases its local entrance only after `onOverlayHidden`; its title and foreground items must not finish while the overlay is still visible. The production Work hold uses the shared Coming soon title entrance and has no spatial readiness dependency.
...
- Home and route-backed Daily Focus loads may release only after fonts, usable route geometry, the active simulation surface, and two render frames are ready. Direct Home boot keeps only a short 12-frame presentation settle buffer; it does not serialize a second long wait after those real readiness checks. Once the identity is staged, supporting groups overlap in one compact entrance instead of waiting for every preceding group to finish. Home gives the canvas-title geometry a bounded grace period, then releases with the semantic DOM title as its visual fallback. `data-abs-home-simulation-ready` and `data-abs-home-canvas-title-prepared` expose those independent states for diagnostics.
...
- Route-in begins surface resolution and route-owned `[data-route-enter]` children from one coordinated readiness-and-paint boundary. Child delays express hierarchy without serializing the entire surface reveal.
...
- Each surface fades once over the shared exit duration, including the wall's spatial copy. Material participants retain local scale ownership. The controls surface uses opacity only; the Home switcher's centring transform and fixed-position containing block remain unchanged throughout departure. Do not run child exit transforms beneath an exiting surface.
- The depth exchange recedes route surfaces to `depthExitScale` (0.976) and resolves incoming surfaces from `depthEnterScale` (0.984). These authored settings drive the existing Instrument Wake tokens. The wall's arrival uses scale only so its material participant retains appearance ownership; marked copy keeps its later entrance at final geometry. Controls use opacity only in both directions. The shell, frame and Button Bar remain outside these targets.
- Surface exchange uses opacity and transform only; it does not animate a full-surface blur. A click during `route-in` starts a fresh `route-out` from the current computed opacity and transform. The interrupted child reveal remains under that fade until the cover is established, so cancelling it cannot expose settled copy. Material exit reverses from the current scale. The committed visible destination is the rollback baseline, and each accepted menu navigation retains its history entry. Repeated clicks on the selected destination are no-ops.
- The shell awaits the participant entrance created after the staged-paint barrier. Abort rejection is handled when that work starts, including a menu click during arrival.
- Each actual title entrance assigns its existing activation times to a fresh letter permutation once. The shared transaction reuses those absolute times during DOM recollection, line reflow and Canvas presentation. Text/semantic order, palette recipe, settled colour, travel and the activation time window stay unchanged. A bounded per-tab history prevents identical consecutive permutations across route remounts and reloads; reduced motion uses the existing settled behavior.
- The persistent Home title plane fades over that same duration before `route-loading` hides the retained bitmap. It remains outside route replacement and the incoming presentation gate continues to own its reveal. Reduced motion uses the shared 120ms opacity handoff.
...
- Work route-in prepares final camera and foreground geometry before reveal. Its title and description retain the shared `identity` and `context` groups; project transforms remain owned by the spatial engine.
- During Work route-in the three-layer depth field may paint the deterministic committed camera frame, but it must not retain an idle animation loop after settlement.
...
## 7) Direct route handoff and readiness

- `RouteTransitionLoader` remains as an invisible input shield and screen-reader status channel inside `#abs-scene`. It renders no spinner, loading copy, plate or other visual interstitial. The Button Bar stays above it and remains responsive. The separate first-document boot overlay is unchanged.
- Home, Work, About and Contact use the persistent backplane throughout navigation. The readiness shield has a transparent background; registered route surfaces own content visibility independently.
- The shell-owned `#simulation-title-canvas` remains outside the registered route-surface list, but it is not outside transition visibility ownership. It schedules through the stable shell animation-frame channel, never the disposable route-runtime scope. Incoming Home `route-loading` keeps it non-paintable, and `route-in` may expose it only after the staged title frame has rendered. A transparent persistent-backplane loader must never reveal its retained bitmap or its full readiness draw.
- The eight-dot spinner belongs only to the separate first-document boot presentation. SPA navigation never mounts that spinner.
- Cold module and data preparation starts while the outgoing scene remains fully visible. Once preparation completes, the scene exits, the destination commits behind the content visibility guard, and route-in begins after readiness and the staged-paint barrier. There is no spinner escalation or artificial minimum wait. A failed preload retains the existing recovery path.
- The route profile keeps the 300ms surface exit, 260ms material exit with up to 40ms stagger, and 330ms destination resolve. Legacy spinner timing fields remain readable for config compatibility but do not delay or render inter-view navigation.
- Visited-route entrance state is memory-only and resets on reload. Reduced motion keeps the same readiness barriers, removes depth and stagger, and uses a 120ms opacity handoff with no visible loader.
...
- `SiteApp` begins data-level preparation for Home, Work, About, and Contact during direct boot without waiting for those jobs before revealing Home. After Home's essential work and direct overlay settle, eligible background time may promote the four routes through the same readiness registry. Hover, focus, pointer-down, and touch intent promote the selected route through that registry.
- The readiness registry key includes route content signature, viewport class, rounded DPR, manual theme, and canonical design-configuration revision. Route modules, normalized shell/runtime/Work configuration, Home copy and active mode module, and the unified Work catalogue are cached only for the current document. Failed promises are removed where retry can recover. Nothing is stored in local storage, session storage, cookies, or the design configuration.
...
- Development Work prewarming loads and validates the unified catalogue plus canonical spatial configuration. It never mounts the camera, depth renderer, drawer, gate, snippet stage, video, or code runtime. Project-detail media and off-screen snippet media are not route-ready dependencies. Production Work prewarming is a no-op while its publication hold is active.
- Home prewarming imports its route and active simulation-mode modules plus cached copy, but never allocates a canvas, initializes balls, or starts the render loop. Contact caches the design-system configuration. About prewarming imports its code-split playback module without mounting the point world or starting its renderer.
...
ABS_DEV_URL=http://localhost:8012 ABS_BROWSER=chromium npm run audit:work-canvas
ABS_DEV_URL=http://localhost:8012 ABS_BROWSER=webkit npm run audit:work-canvas
npm run audit:work-publication
ABS_DEV_URL=http://localhost:8013 npm run certify:screens
```

Run `npm run check:route-transitions` for the fast deterministic transaction suite before browser work. Transition trace JSON reports phase durations, loader-covered duration, readiness timing, first meaningful destination content, maximum frame interval, Long Task API entries where the browser supports them, and the recorder's own maximum/total per-frame sampling cost. Continuous computed-style recording is intentionally intrusive; treat its cadence as invariant evidence rather than an uninstrumented frame-rate benchmark. Use `ABS_TRANSITION_CPU_THROTTLE_RATE=4` for the constrained Chromium profile and compare like-for-like reports. Reduced-motion checks preserve the readiness barriers and opacity handoff, with no spatial motion, inter-view spinner or artificial minimum hold. The Work audit owns centre-to-open and overlay evidence; do not substitute the retired linear-carousel audit for it. Automated checks do not replace follow-up on physical iPhone/Safari and Android/Chrome hardware.
...
- [ ] The shared route-title contract passes in Chromium and WebKit: Home, Work, About, and Contact retain their centred live geometry, captured colour/opacity endpoint, and animation clock through desktop, short-landscape, and phone resizes, with no cleanup jump.
...
- [ ] First readable Home -> Work frame has final camera/title/item geometry and no geometry snap afterward.
- [ ] Continuous-RAF Chromium/WebKit audits pass, plus focused mobile, reduced-motion, rapid-intent, and delayed-readiness probes.
- [ ] Work, About, and Contact returns to Home keep the title plane covered after commit, begin route-in from a staged blank title revision, and settle with one visible source-connected title plane.
...
### Home title settlement

The hidden Home title is a fixed measurement source for the persistent Canvas. Its shell hero surface fades without a transform; the visible Canvas and simulation own depth movement. Canvas keeps the same per-letter shaping before and after settlement, including Atmosphere previews, so completing a title cannot change kerning or refresh a scaled endpoint. Canvas-owned title lines retain their final opacity through deferred style cleanup, preventing a one-frame full-opacity flash. Activation order is cached from the combined letter sequence rather than its current line containers, so wrapping does not create a second shuffle.
