...
- `15` modes are in the narrative cycle (`NARRATIVE_MODE_SEQUENCE`).
- The circular Daily Simulation switcher and live selection are derived from the catalog `daily-rotation` stage; the current catalog contains `10` Daily simulations.
...
- `3d-sphere` (Continuity) and `3d-cube` (Scaffold) are disabled. Their implementations remain available for internal review, but they are excluded from Daily Simulation and the narrative sequence because their 3D point-cloud structures differ from the shared Home simulation language.
...
| Continuity | `3d-sphere` | Disabled; internal review only |
| Scaffold | `3d-cube` | Disabled; internal review only |
| Perspective | `starfield-3d` | Legacy implementation; outside the catalog |
...
2. `flies` — ATTENTION
3. `water` — FLOW
4. `repel-room` — TENSION
5. `flock-of-birds` — CONVERGENCE
6. `flubber-blob` — COHESION
7. `kaleidoscope-3` — REFRACTION
8. `magnetic` — MAGNETIC
9. `kaleidoscope-rift` — MULTIPLICITY
10. `particle-fountain-b` — CADENCE

### Daily simulation sound map

Sound follows a visible physical event and stays silent during idle animation. `pit`, `water`, and `magnetic` use collision sound; `flies` uses only meaningful wall impact; `repel-room` uses pressure; `flubber-blob` uses aggregated soft-body impact; and `particle-fountain-b` keeps its phrase cues. `flock-of-birds`, `kaleidoscope-3`, and `kaleidoscope-rift` are silent.

### Extended Tier (shown after Featured)

11. `aperture-bloom` — APERTURE BLOOM
12. `weightless` — WEIGHTLESS DRIFT
13. `critters` — CRITTER SWARM
14. `pressure-crucible` — PRESSURE FIELD
15. `particle-fountain` — FOUNTAIN A
...
- The disabled `3d-sphere` and `3d-cube` implementations retain their depth-plane title layering for internal review. Both split particles around the fixed title plane at normalized z `0.5`, with some circles behind the title and others in front.
...
- `3d-sphere`: disabled internal-review implementation of a dense breathing spherical point cloud with rotating depth layers and trackball response.
- `3d-cube`: disabled internal-review implementation of Scaffold’s breathing, rotating, and pointer-tumbled 3D cube point cloud, with a face lattice, perspective scaling, and depth fog.
- `starfield-3d`: legacy implementation outside the current catalog; denser depth-projected field with larger far-depth circles, recycle, pointer/touch camera pan, and shared distance fog.
