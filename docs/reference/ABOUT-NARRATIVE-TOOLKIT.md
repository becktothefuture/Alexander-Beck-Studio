...
A native scroll container maps eleven source beats to the camera rail. The Blender baseline is 32 viewport lengths. The canonical text-motion duration scale gives title sections 75% more native scroll distance, bringing the current baseline to 36.8 viewport lengths. Measured prose can extend reading beats so every line can enter and leave the viewport. This does not shorten either tunnel.
