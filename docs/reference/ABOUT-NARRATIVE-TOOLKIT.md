...
The camera follows evaluated Blender position and rotation. A critically damped spring gives the camera a gentle glide after scrolling, covering 95% of an impulse in about 600 ms. Slow frames retain the easing instead of snapping to the scroll target. A monotone timing curve joins the different section speeds continuously, starts gently and eases into the final camera stop. Every authored beat boundary stays exact; copy, title holds and history retain native progress. The spring settles exactly on that camera target without overshoot. Reverse scrolling retraces the path; history restoration and reduced motion bypass the glide. Moving assemblies have a separate ambient clock. They keep moving during a scroll hold and stop when the page is hidden or reduced motion is enabled. The surface contract supports ambient object loops only; scroll-bound object animation is not an exposed control.
