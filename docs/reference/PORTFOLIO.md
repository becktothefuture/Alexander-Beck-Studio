...
Card sizing has one responsive rule: card width is 25% of the measured deck stage, bounded by the authored `220px` minimum and `420px` maximum. A short desktop may resolve to a smaller width inside those same bounds so the title, card, and bottom clearance all remain visible. Height is always derived from the `316:461` aspect ratio; there are no separate mobile, desktop, height, or large-viewport scales. Card copy and corner geometry scale directly from the resolved card width once it exceeds the original `316px` reference width.

The track seat is height-responsive. Its authored desktop and mobile centre endpoints remain available, while the measured title block supplies the minimum card centre needed to keep the complete intro and active card separate. Desktop keeps its `3dvh` track offset, while the title group uses its unshifted responsive inset. This makes the title and larger card one composition without a second large-screen cap or group-offset system. On narrow portrait viewports, the intro inset follows the available window height and the pagination track remains anchored near the bottom of the studio window. The three regions must keep visible separation without borrowing space from the persistent Button Bar.

The development panel exposes two signed composition offsets after that responsive layout resolves: **Slider Y** moves the complete card track and its particle-field quiet band, while **Title group Y** moves the Work title and description as one unit. Both are authored in `dvh`, default to zero, and preserve the automatic responsive baseline when untouched.
...
The field has a soft horizontal quiet band centred on the effective responsive Track Y value. Its nine authored controls cover idle/fast opacity, quiet-band height/opacity, density, far/near circle size, motion response, and parallax depth. During route-in it paints one deterministic static composition without scheduling drift; movement resumes only after the route returns to idle. It clears or pauses while the route is hidden, drawer-open, explicitly suspended, or unmounted. It does not own the legacy `#c` canvas.
