...
Placement is derived from stable item identity, `placementOrder`, the selected preset, `projectSpacing`, and `layoutSeed`. Higher project spacing pushes the deterministic candidates farther from the title and from one another. The modulo period is then calculated from those expanded project extents, so the repeat area grows with the composition instead of clipping or repeating projects too soon. Existing placements are append-stable when new items are appended and earlier placement inputs stay unchanged. The world remains quantized to the grid.
...
| `itemGapCells` | `2` | 1–6, step 1 |
| `itemScale` | `1` | 0.75–1.35, step 0.01 |
| `sizeVariation` | `0.28` | 0–0.5, step 0.01 |
| `labelGapPx` | `8` | 4–16, step 1 |
| `dotRadiusPx` | `2.25` | 2–7, step 0.25 |
| `dotOpacity` | `0.28` | 0.12–0.6, step 0.01 |
| `colorWakeRadiusPx` | `168` | 56–360, step 8 |
| `colorWakePersistenceMs` | `1200` | 300–3000, step 100 |
| `colorWakeOpacity` | `0.88` | 0.25–1, step 0.01 |
| `colorWakeDensity` | `1` | 0.15–1, step 0.05 |
| `colorWakeEdgeSoftness` | `0` | 0–1, step 0.05 |
| `colorWakeDotScale` | `1` | 0.5–1.5, step 0.05 |
