...
`shell.surface.simulationBodyMaterial.enabled` is canonically `false`. The code default is also `false`, so a missing or incomplete configuration cannot turn the sphere finish back on. Production bodies keep their active palette colour and use the existing flat render path across Home, Work / Portfolio, About, Contact, and the Home quote puck. The Lab dot field remains a separate neutral grid material.
