...
The development cog and `/` shortcut are page-owned listeners registered through `addStableEventListener`; Home runtime cleanup must not remove them. Dock visibility resolves the current connected `#panelDock`, recreating a missing host on activation. The launcher stays above the dock on short screens. Verify with `node --test scripts/check-editor-listener-lifecycle.mjs` and `node scripts/audit-dev-panel.mjs` (`ABS_BROWSER=webkit` for WebKit).
...
| Tactile navigation | `runtime.tactileNav*`, `src/lib/buttonBarControls.js`, `--tactile-nav-*` | Four stationary faces below the window. Full target-height reserve, independent pressure and selection, palette slot colours and shading-only depth/rebound. |
...
The **Menu → Tactile navigation** section exposes face and icon size, mobile/desktop padding, gap and label sizes, shading depth, rebound and navigation volume. **Group top padding** and **Group bottom padding** independently adjust the outer row spacing from 0–32px, with 8px/4px defaults. The window reserve updates with them; touch targets and safe-area spacing remain intact. Desktop horizontal padding spans 16–80px; mobile spans 12–60px. The outer gutter caps at 40px so extra padding widens the faces without crowding out touch targets. Collapsed **Resting light and shadow** and **Pressed light and shadow** groups expose edge-light and shadow strength and softness. Upper recessed shadow strength remains controlled by **Pressed depth**. **Advanced timing** is collapsed. The scene light direction is set in Surface Finish and stays fixed during pressure and release. The authored illuminated-silicone finish adds **Internal light** and **Colour falloff** to the pressed group. They shape an opaque, palette-derived gradient inside the face. The lower white catch defaults to zero; broad, low-strength edge shading replaces the heavy lower roll. Lighting stays fixed during pressure and release. Horizontal padding remains editable; a bounded viewport adjustment and flex compression fit the faces to the available row width. The site-sound checkbox aliases the shared engine preference rather than saving a second navigation mute. Canonical settings, runtime save/reset and Design Mode all use the same registry. The existing Palette selector previews an approved ball palette for the current editor document; it updates the balls and navigation together and resets to the schedule on reload.
...
| Material | Consumers |
| --- | --- |
| Window & Utility Rail | Contact blend, inner rim, exterior reflections and the continuous curved rail join |
| Menu items | Resting/pressed rubber faces, internal colour field, highlights and cast shadows |
| Balls & quote puck | Shared cached sphere sprites/atlases and the puck's DOM edge and shadow |
| Buttons inside the window | Shared labelled actions and circular utilities, including Change effect, close and copy email |
| Cards & sheets | Work cards, preview stages, gate panels, project sheets and editorial card shadows |

The authoritative model is `src/lib/scene-lighting.js`. Configuration is stored at `shell.surface.lighting.light` and `.dark` in `design-system.json`. Each profile has `intensity`, `direction`, `softness`, `thickness`, and `materials` keyed by `window`, `navigation`, `balls`, `buttons`, and `cards`. Scene values multiply each material's response. Runtime CSS and cached sphere baking consume the same normalized model.

Legacy `surfaceEdgeSoftness`, `surfaceEdgeDepth`, `surfaceLightIntensity`, and `surfaceLightDirection` values migrate into both profiles once and are removed from authored/save output. The `--surface-*` CSS names remain derived window aliases. Live edits update the current shell object; docked and detached hosts share that owner. **Save Design JSON**, reload, canonical normalization and generated `shell-config.json` use the same rig. Production embeds the normalized saved configuration.

100% and Even preserve the calibrated reference finish. Even keeps the balanced window, overhead menu cue, diagonal action edge and existing sphere lighting. Directional settings align the light source and opposing shading across materials. Geometry, collision shapes, radii, frame colour, focus rings and interaction motion retain their existing owners.

The original window calibration remains in collapsed **Fine tuning**. It retains fractional values, separate theme strengths and mobile far-glow endpoints. Menu and Ball Material retain their detailed calibration controls. These form the underlying material; the central rig controls its response. The lowest softness settings retain a small blur on the window to prevent a sharp perimeter stripe. No dark perimeter stroke is added.
