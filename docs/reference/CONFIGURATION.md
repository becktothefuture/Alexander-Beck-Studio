...
The **Ball Contrast Veil** uses the existing `simulationContrastVeil*` runtime keys. On Home it is rendered inside the ball canvas by `drawBallContrastVeil()`, tinting only visible ball/pebble bodies toward the active wall background colour with deterministic dither. The DOM hook `.simulation-contrast-veil` stays inert on both Home and Portfolio. Portfolio reuses its existing geometry-aware card edge factor instead: intersecting cards blend from full opacity toward a `0.8` floor, while the independent offscreen guard fade still reaches zero before ring rebasing. Portfolio also suppresses `#simulations::before`; the thin `.inner-wall-gradient-edge` rim remains.
