...
Portfolio config applies to the portfolio pit and the **wall-contained project bottom sheet** (not a fullscreen takeover). `runtime.motion.openDurationMs` drives sheet slide in/out; `colorFloodHoldMs` is unused in this layout but remains in generated JSON for compatibility. The project dialog (`#portfolioProjectView`) is inserted by `portfolio/app.js` **`createProjectView()`** into **`#portfolio-sheet-host`** when that node exists (`StudioShell.jsx`); same fixed inset and outer radius as `#bravia-balls` (`--safari-tint-inset`, `--frame-outer-radius`). With `body.portfolio-project-open`, the host uses **z-index 260** so the sheet stacks above `.fade-content` (header/footer) and the quote viewport host. **Stacking order is mandatory product behavior** — see **`docs/reference/LAYER-STACKING.md`**. Close uses an **inline SVG** (subset icon fonts can omit `ti-x`). Pit labels stay in `#portfolioProjectMount`. Without the host, the dialog falls back to the mount using **`--portfolio-drawer-radius`**. The scroller hides scrollbars (CSS). Inertial / weighted scroll uses **`lenis`** on `.portfolio-project-view__scroller` unless `prefers-reduced-motion: reduce` (native scroll then). The hero image is clipped with **`--frame-inner-radius`** and **`--frame-border-width`** inset to match **`#bravia-balls canvas`**; hero block height follows the scrollport (**`100cqh`** with **`container-type: size`** on the scroller, `min(100dvh, 100svh)` fallback).
