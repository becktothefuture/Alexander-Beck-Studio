...
| Stable dark Button Bar | `buttonBarBottomInsetPx`, `buttonBarWindowOverlapPx`, `buttonBarRouteGapPx`, `buttonBarDesktopPaddingScale`, `buttonBarDesktopFontScale`, `buttonBarActiveInsetPx`, `buttonBarActiveDepthPx`, the active-surface opacity controls, and `--button-bar-*` projection tokens | Route labels, the shared moving active key, and the fixed sound/theme toggle pair keep the dark capsule material across routes and themes. `buttonBarActiveInsetPx` applies equally to all four key edges. Route separators are intentionally absent. The derived frame reserve is `height - overlap + bottom inset`; desktop route padding and type are the authored scales times their mobile values. |
