...
The panel groups controls under **World**, **Work**, **Dot field**, and **Motion**. It also provides recenter, deliberate seed generation, reset, and save actions. Diagnostics are read-only and must not enter `design-system.json`. Exact ownership, performance limits, and placement rules are recorded in [`PLAYGROUND.md`](PLAYGROUND.md) and enforced by the route normalizer.

Live changes are provisional. **Generate new seed** deliberately recomposes the whole field. **Save design configuration** writes the normalized `playground` snapshot through `performDesignSystemSave()`. When the write-capable local endpoint is unavailable, the established save flow downloads the JSON instead. Reload, root build flattening, and preview must preserve every Work spatial field.
