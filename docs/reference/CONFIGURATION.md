...
**Project card colors:** Live scroll cards use explicit per-project material colors in `portfolio/app.js`, exposed as `--portfolio-card-*` variables on each card. `getPortfolioProjectPaletteColor` remains a fallback for legacy/archived physics paths.
...
**Legacy collision notes:** Archived pit paths set `ballSpacing` to **0** and `wallInset` to **0** on load, then reset `ballBallSurfaceGapPx` / `collisionPairSlopPx` on destroy so the index pit keeps ratio-only `ballSpacing` from config. The live scroll rail does not depend on these physics values.
