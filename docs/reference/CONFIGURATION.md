...
| Stable dark Button Bar | `buttonBarBottomInsetPx`, `buttonBarWindowOverlapPx`, `buttonBarRouteGapPx`, `buttonBarDesktopPaddingScale`, `buttonBarDesktopFontScale`, `buttonBarIndicatorSizePx`, and `--button-bar-*` projection tokens | Route tabs, dot, dividers, and the fixed sound/theme toggle pair keep the Figma capsule material across routes and themes. The derived frame reserve is `height - overlap + bottom inset`; desktop route padding and type are the authored scales times their mobile values. |
