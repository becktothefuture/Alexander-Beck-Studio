...
Dark/light theme is one site-wide content state, but its visual reach stops at the studio-window interior. Route components, simulations, gates, and in-window overlays consume that state; they must not create their own theme policy. The exposed wall/frame, browser chrome, persistent Button Bar, and Utility Rail are outer-shell systems and do not consume the manual site theme.

The intended surface hierarchy is summarized in [`DESIGN.md`](../../DESIGN.md). The runtime, first-paint mirror, configuration, theme-color projection, Utility Rail, and browser audits implement the same boundary.
...
- `light` and `dark` are explicit user overrides at every viewport width.
- The Utility Rail exposes the same theme action on desktop and mobile.
- Route, viewport width, local time, and content type must not override the preference.
...
- the runtime theme event detail (`{ theme, effectiveTheme, isDark }`).
...
The early inline script in each boot-overlay HTML entry is a first-paint mirror, not a second preference owner. It resolves the same stored preference before React loads. Changes to preference behavior must update every boot-overlay entry and the runtime controller together.
...
| Persistent Button Bar | Fixed outer-shell capsule tokens | Theme-invariant gradient capsule with one moving active key |
| Persistent Utility Rail | Fixed outer-shell rail tokens | Theme-invariant vertical capsule with live theme and sound controls |
...
- A browser/OS scheme change may update an Auto studio window without changing the frame or overriding an explicit preference.
...
## Button Bar and Utility Rail

The Button Bar crosses the studio window's bottom edge but remains owned by the dark outer shell. Its `#141414` to black capsule, inset highlights, inactive icon-label pairs, and moving graphite key are invariant across site themes. Tabs never borrow `--studio-window-bg` or add independent selected surfaces.

The separate Utility Rail attaches to the studio-window right edge and remains owned by the same dark outer shell. It shows theme above sound at every viewport width. Theme changes may change the theme icon, behavior, and accessible label, but must not recolour or resize the rail material. There is no slider track or separate automatic-theme reset control.
