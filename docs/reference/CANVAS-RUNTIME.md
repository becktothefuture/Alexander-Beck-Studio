...
- manual theme Afterglow samples one shared `500ms` material timeline for intensity and colour strength while the window surface settles in `313ms`; theme changes still clear feedback and prime a fresh composite. Feedback memory is zero during the lighting transition so it cannot extend the tail. The shell title plane ignores transient background, grain, and rim style updates, but redraws for ink changes;
