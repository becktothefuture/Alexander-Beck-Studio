...
- Interaction: native click/Enter activation, separate transient pointer pressure, single pending/selected latch, cancellation cleanup, browser-history restoration and visible keyboard focus. The authored pressed depth, lower shadow and shading rebound are zero: pressing lights the face without adding a dark inset shadow or a shadow-release animation.
...
- Active colour uses a broad, stationary internal-light gradient, derived from the live palette. Internal light (0–0.12) and Colour falloff (0–0.4) are in **Pressed light and shadow**. The upper and lower pressed shadows are switched off in the authored settings. All light stays inside the existing silhouette; the resting material and window stay unchanged.
...
- Width: the four equal-width buttons fill 80% of the viewport, leaving 10% on each side, up to a centred 720px group maximum. **Menu → Tactile navigation → Group maximum width** adjusts the cap (320–960px). Safe-area insets remain respected, and button gaps, face heights and label positions stay independent of width.
...
- Position: fixed to the studio-window right edge; desktop and mobile target a `20svh` centre. Both apply a `10rem` top-text reserve plus the existing control clearance. Mobile increases the text reserve to `13rem` on the narrowest screens to accommodate wrapping. The reserve persists across routes to keep the shell stationary.
...
- Configuration: the top-level **Utility Rail** group exposes separate **Desktop** and **Mobile** geometry, including **Vertical Position** (Desktop 10–90%, Mobile 10–90%). **Padding** (0–32px) and **Corner Radius** (0–80px, in 0.5px steps) adjust the integrated bay separately for each viewport group. Radius and vertical position are constrained by the available window space; the button faces and icons keep their existing styling.
