...
The Home switcher advances one effect in circular catalogue order. Its visible label stays `Change effect` beside a shuffle glyph, while the accessible name and polite status carry the current effect name. The control is temporarily unavailable during the existing simulation transaction and adds no label handoff, width animation, or icon rotation. Contact/About copy holds its width while the email label rotates to Copied/check or Copy failed. The Contact background ripple still starts on activation, but the retired button-local colour wash is removed. The standard custom cursor remains visible over every control and uses the same smaller, quieter clickable state it uses elsewhere.
