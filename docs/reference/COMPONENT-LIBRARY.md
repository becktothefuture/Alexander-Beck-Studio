...
The Home switcher advances one effect in circular catalogue order. Its visible label stays `Change effect` without an icon, at 90% of the shared labelled-action height, with balanced 16px inline padding and no reserved icon gap. The accessible name and polite status carry the current effect name. The control is temporarily unavailable during the existing simulation transaction and adds no label handoff or width animation. On mobile, equal side tracks centre the footer caption between the social controls and London time. Contact/About copy holds its width while the email label rotates to Copied/check or Copy failed. The Contact background ripple still starts on activation, but the retired button-local colour wash is removed. The standard custom cursor remains visible over every control and uses the same smaller, quieter clickable state it uses elsewhere.
