...
The Home switcher advances one effect in circular catalogue order. Its visible text-only label is `CHANGE EFFECT`: 10.5px regular-medium Geist with `0.09em` tracking inside a 36px capsule with balanced 20px inline padding, no reserved icon gap, and a half-pixel upward optical baseline correction. A 44px invisible hit area surrounds the translucent glass fill. The shared 16px backdrop blur and paired light/shadow inset edges remain visible in both themes, while softer secondary ink and the absence of an outer outline keep the action quiet. Hover and focus keep the switcher stationary and use material/ink changes only. Press moves the complete capsule and label down by 1px into the page and strengthens the ink without changing the resting fill or reversing the inset light/shadow direction; the acknowledgement persists through the simulation transaction without scaling or bounce. The accessible name and polite status carry the current effect name. The control adds no label handoff or width animation. On mobile, equal side tracks centre the footer caption between the social controls and London time. Contact/About copy holds its width while the email label rotates to Copied/check or Copy failed. The Contact background ripple still starts on activation, but the retired button-local colour wash is removed. The standard custom cursor remains visible over every control and uses the same smaller, quieter clickable state it uses elsewhere.
