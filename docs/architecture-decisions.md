...
- A focused mode-button dependency seam reduced the active legacy cyclic component from 12 modules/23 internal edges to 9/15. A route-neutral scene-pointer event port then reduced it to 5/8. Finally, a mode-runtime bridge let the controller publish loaded hooks while physics consumed them without importing the controller, eliminating the active cycle without moving frame-frequency work into React. The fail-closed graph and mutation checks remain canonical; this is not approval for a broad runtime rewrite.
