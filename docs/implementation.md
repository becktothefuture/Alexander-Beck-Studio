...
**Build:** from repo root, `npm run build` runs `flatten:design-config` then Vite (`react-app/app/`). GitHub Pages calls the same root build entrypoint (see `.github/workflows/gh-pages.yml`).
