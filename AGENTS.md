...
- **Daily simulation contract:** `docs/reference/SIMULATION-DESIGN-GUIDELINES.md` §9 is canonical for promoted Daily Simulation rendering, naming, route-backed runtime shape, and switch-transition behavior. Daily visuals use flat palette fills, no local shadows/lights/rims, shared wall ownership, clean home URLs, and draw-only transition repaints.
...
- **Scene entrance principle (inside-window animation boundary):** `docs/reference/SCENE-ENTRANCE-PRINCIPLE.md` is canonical. Route-owned content inside the studio window may enter using the homepage depth-emergence language; the physical window, outer frame, Button Bar, and other persistent outside-window shell surfaces are permanent. Never use unequal X/Y scaling or skew for visual handoffs, and begin parallax only after shared geometry has settled.
...
- **Homepage layer principle:** Think of the homepage as a deliberate back-to-front layer stack, not a flat pile of absolute elements: **Base Frame** → **App Scene Transform Group** → **Simulation Wall + Scene Effects** → **Ball Canvas** (balls plus the visual homepage title/subtitle when rendered through the canvas/depth path; keep a semantic DOM title as source/accessibility) → **Contrast Inner-Shadow Veil** (edge contrast layer above canvas, below UI, pointer-transparent) → **UI Layer** (top chrome, simulation switcher, center links, footer) → **Overlay Layer** (portfolio sheet + quote viewport hosts) → **Modal Layer**. New layer work should use semantic names and preserve this order; treat existing `abs-*` names as compatibility surfaces unless every consumer is deliberately migrated. Implementation prompt: `docs/prompts/homepage-layer-stack-semantic-rename-implementation.md`.
...
- **Daily focus URL contract:** Public home/daily simulation state must settle on the clean home route (`/` or `/index.html`). Do not leave `/lab/<id>.html?daily=1`, `?mode=`, `?focus=`, or `?simulation=` in the URL after boot or simulation-switcher selection; those forms are backwards-compatible/debug inputs only.
- **Daily simulation material/transition contract:** See `docs/reference/SIMULATION-DESIGN-GUIDELINES.md` §9 before changing Daily Simulation renderers. Route-backed daily repaint callbacks must draw current state only during `simulation-focus` transitions; normal animation frames own physics/time updates.
- **Wall/frame protection:** Never alter `#simulations` wall geometry, frame radii, wall-shadow plate behavior, `.inner-wall-gradient-edge`, `.frame-vignette`, or shell wall colors unless explicitly asked. If touched, compare against the approved wall-shadow baseline and verify corners, light edge, and frame separation before reporting completion.
- **Wall/frame color separation:** Do not set wall, site frame, browser/Safari frame, and page background tokens to the same color unless explicitly requested. Preserve visible separation between browser frame, outer wall, inner wall, and canvas surface.
- **Busy simulation chooser surfaces:** Simulation switcher and chooser surfaces that sit over dense simulations need a blurred/translucent resting ground plus the shared solid `--cursor-color` hover/focus/active object treatment.
