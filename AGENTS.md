...
- **Portfolio CSS on SPA:** `index.html` must include **`/css/portfolio.css`** (not only `portfolio.html`). Pit/chrome rules may use `body.portfolio-page`; **`#portfolio-sheet-host`** is inset like the pit canvas (`--safari-tint-inset` + `--frame-border-width`), clip uses **`--frame-inner-radius`** (same as `#bravia-balls canvas`); **`corner-shape: round`** is forced under the host so drawer clips + nested radii stay concentric (clip-path `round` is circular). Open z-index applies on **all** routes so the drawer works from the home pit too.
