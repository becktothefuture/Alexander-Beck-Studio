...
// ║     - Button-origin outward emission for power-transfer sync                 ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
 ...
 * @param {object} options - Optional tuning (particleScale, emissionCenter)
 */
export function triggerCursorExplosion(x, y, color, velocity = 0, options = {}) {
  const g = getGlobals();
  ...
  const particleScale = clamp(options?.particleScale ?? 1, 0.1, 1);
  const particlesToCreate = Math.floor(baseCount * velocityFactor * particleScale);
  ...
  const emissionCenter = options?.emissionCenter;
  if (emissionCenter && (emissionCenter.x !== x || emissionCenter.y !== y)) {
    spreadCenter = Math.atan2(y - emissionCenter.y, x - emissionCenter.x);
  } else if (lastMouseDirX !== 0 || lastMouseDirY !== 0) {
