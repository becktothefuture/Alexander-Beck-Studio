...
class PortfolioApp {
  ...
  renderSlides() {
    ...
    for (let slot = 0; slot < slotCount; slot += 1) {
      ...
      slide.style.setProperty(
        '--project-circle-fill',
        PROJECT_CIRCLE_PALETTE[projectIndex % PROJECT_CIRCLE_PALETTE.length]
      );
  ...
  destroy() {
    ...
    if (this.wheelFrame) {
      window.cancelAnimationFrame?.(this.wheelFrame);
      this.wheelFrame = null;
    }
    ...
    if (this.detailOverlay?.parentNode) {
      this.detailOverlay.parentNode.removeChild(this.detailOverlay);
    }
