...
export function resolveRollercoasterProjection(source, width, height, {
  ...
  if (![width, height, horizontalFov, portraitVerticalFov].every(Number.isFinite)
    || width <= 0 || height <= 0 || horizontalFov <= 0 || horizontalFov >= 180
  ...
  const verticalFov = Math.min(portraitVerticalFov,
    2 * Math.atan(Math.tan(horizontalFov / DEGREES / 2) / aspect) * DEGREES);
...
export function resolveRollercoasterSamplingSpacing(radiusWU, {
  baseSpacing = ROLLERCOASTER_FIELD.spacing, currentSpacing, density = 1,
} = {}) {
  ...
  if (!Number.isFinite(density) || density < 0.5 || density > 2) throw new RangeError('About density must be between 0.5 and 2.');
  const diameter = radiusWU * 2 / Math.sqrt(density);
