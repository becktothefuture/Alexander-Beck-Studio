...
**Acceptance Criteria:**
- [ ] `--studio-window-bg` and `--frame-inner-surface` resolve to the authored light or dark content background.
- [ ] Home, Portfolio, About Me, and Contact show legible theme-appropriate text and controls.
- [ ] The inner contrast veil follows the active window surface.
- [ ] Verify in a real browser with light/dark screenshots for all four routes.
...
**Acceptance Criteria:**
- [ ] At a fixed browser/OS colour preference, manual site-theme changes leave `--abs-browser-chrome`, `--frame-color`, `--wall-color`, and sampled outer pixels unchanged.
- [ ] Safari/theme-color-capable browsers retain the authored frame palette.
- [ ] Locked desktop Chromium and Firefox retain their browser-native frame palettes.
- [ ] Browser/OS colour-preference changes can update outer harmony without overriding a manual site-theme preference.
- [ ] Verify in Chromium and WebKit; verify the Firefox resolver contract when a Firefox runtime is unavailable.
...
**Acceptance Criteria:**
- [ ] Route-tab and secondary-control surfaces do not inherit inner-window theme ink.
- [ ] Home, Portfolio, About Me, and Contact tabs remain legible in both site themes.
- [ ] The correct active route remains indicated after SPA navigation and reload.
- [ ] Button Bar geometry and responsive layout do not change.
- [ ] Verify in desktop and mobile browser captures.
...
**Acceptance Criteria:**
- [ ] Theme, palette-surface, wall-invariance, and outer-frame audits assert the separated contract.
- [ ] `docs/reference/THEME-STATE.md`, `docs/reference/CONFIGURATION.md`, and `AGENTS.md` describe the same boundary.
- [ ] `npm run check:site` passes.
- [ ] Chromium and WebKit theme-consistency audits pass against a production preview.
