...
| --- | --- | --- | --- | --- | --- |
| 2026-07-08 | `prd-content-label-source-alignment.md` | Canonical About/CV label is `About Me`, owned by `contents-home.json`. | Matches component-library title-case guidance and lets active routes share one source. Portfolio legacy template cleanup is deferred to PRD 8, which owns that file's support/retirement decision. | `contents-home.json`, `HomeRoute.jsx`, `PortfolioRoute.jsx`, `CvRoute.jsx`, `SITE-COPY.md`, and dev-browser route inspection. | None. |
| 2026-07-08 | `prd-portfolio-legacy-template-retirement.md` | Retain `PortfolioPage.jsx` and `templates/portfolio-body.html`; do not delete or archive them in this pass. | The PRD requires explicit user approval before deleting or archiving those files. The files are not active route entries, but the template is still validated and imported by the retained page shell. | `rg` import graph, `docs/reference/PORTFOLIO.md`, `validate-html-fragments`, `check:site`, `certify:screens`, and `audit:portfolio-gate`. | Explicit approval if future work should delete or archive the retained standalone surface. |
